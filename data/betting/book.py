...
class BookmakerSimulator:
    ...
    def get_latest_odds(self):
        ...
        # Get latest
        odds = odds.loc[odds.groupby(["match_id","selection_id"])["request_time"].idxmax()]
        return odds
    ...
    def place_bet(
        ...
        # Check the validity
        assert selection_id in odds.loc[odds["match_id"] == match_id, "selection_id"].values, f"Invalid selection_id {selection_id} for match_id {match_id}."

        assert bet_amount > 0 and bet_amount < 1000.0, "Invalid bet amount - valid range [0,1000]."
        ...
        with closing(sqlite3.connect(self.bets_db_path)) as bets_db_conn:
            with closing(bets_db_conn.cursor()) as cursor:
                cursor.execute('''
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (bettor_id, match_id, selection_id, runner_name, runner_type, back_or_lay, bet_amount, selection_odds, "PLACED"))
                bet_id = cursor.lastrowid
            bets_db_conn.commit()

        print(f"Bet placed: {back_or_lay} {bet_amount} on {runner_name}. Bet id {bet_id}.")
        return bet_id
    ...
    def cancel_bet(self, bettor_id, bet_id):
        ...
        assert bet_id in all_bets.loc[all_bets["bettor_id"] == bettor_id, "id"].values, f"Invalid bet id {bet_id} for bettor {bettor_id}"
    ...
    def resolve_bets(self, match_id, winning_selection_id):
        ...
        # Check the validity
        assert winning_selection_id in odds.loc[odds["match_id"] == match_id, "selection_id"].values, f"Invalid selection_id {winning_selection_id} for match_id {match_id}."

        runner_outcome = odds.loc[(odds["match_id"] == match_id)&(odds["selection_id"] == winning_selection_id), "runner_type"].values[0]
        # Winning back bets
        with closing(sqlite3.connect(self.bets_db_path)) as bets_db_conn:
            with closing(bets_db_conn.cursor()) as cursor:
                # Winning back bets
